...
// Transforms and integrates a vertex map into the volume
__kernel void integrateData(__global int* voxels,
        __read_only image2d_t vertex_map, float translation_x,
        float translation_y, float translation_z, float rot_x, float rot_y, float rot_z)
{
        uint x = get_global_id(0);
        uint y = get_global_id(1);
        float4 vertex = (float4) (x, y, 0.0f, 0.0f);//read_imagef(vertex_map, sampler, (int2) (x, y));

        uint width = get_image_width(vertex_map);
        //uint value = read_imageui(vertex_map, sampler, (int2) (x, y)).x;
        //uint voxel_index = (int) (vertex.z * width * width) + (int) (vertex.y * width)  + (int) (vertex.x);
        uint voxel_index = (int) (20 * width * width) + (int) (y * width)  + (int) (x);
        voxels[voxel_index] = 255;
}

bool intersect(float3 ray_origin, float3 ray_direction, float3 box_a, float3 box_b, float3* box_intersection)
{
        // ?? To do: Avoid branches in the rendering code
